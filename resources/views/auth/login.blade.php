<x-guest-layout>
    <div class="space-y-7">
        <div class="space-y-2">
            ...
            <h2 class="text-3xl font-extrabold text-kanca-dark leading-tight">Welcome back.</h2>
            <p class="text-sm text-gray-500">Login untuk melihat dashboard, event pass, menu favorit, dan portal sesuai role akunmu.</p>
        </div>
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-5">
            ...
            <div>
                <div class="flex items-center justify-between mb-2">
                    <label for="password" class="block text-xs font-extrabold text-gray-700 uppercase tracking-wide">Password</label>
                    @if (Route::has('password.request'))
                        <a class="text-xs font-bold text-kanca-teal hover:text-kanca-orange transition-colors" href="{{ route('password.request') }}">Forgot password?</a>
                    @endif
                </div>
                <input id="password" type="password" name="password" required autocomplete="current-password" placeholder="Enter your password" class="w-full rounded-2xl border border-gray-200 bg-kanca-bg/70 px-4 py-3 text-sm text-kanca-dark placeholder:text-gray-400 focus:border-kanca-orange focus:ring-kanca-orange">
        ...
        <div class="rounded-2xl bg-kanca-bg border border-orange-100 p-4 text-xs text-gray-600 space-y-2">
            <p class="font-extrabold text-kanca-dark">Demo accounts</p>
        ...
        <p class="text-center text-sm text-gray-500">
            New to Kanca?
            <a href="{{ route('register') }}" class="font-extrabold text-kanca-orange hover:text-kanca-teal transition-colors">Create an account</a>
        </p>
    </div>
</x-guest-layout>
