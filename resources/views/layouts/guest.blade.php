...
<title>{{ config('app.name', 'Kanca Coffee') }} - {{ __('Account Access') }}</title>
    <meta name="description" content="{{ __('Login or register to join Kanca Coffee community events, save favorite menus, and manage your account.') }}">
...
<a href="{{ route('home') }}" class="inline-flex items-center gap-3 w-fit">
                    <x-logo class="w-11 h-11" rounded="rounded-2xl" />
...
<div class="max-w-2xl space-y-6">
                    ...
                    <h1 class="text-5xl xl:text-6xl font-extrabold leading-tight">{{ __('Masuk ke ruang komunitas Kanca Coffee.') }}</h1>
                    <p class="text-white/80 text-base leading-relaxed">
                        {{ __('Simpan menu favorit, daftar event, pantau tiket komunitas, dan nikmati pengalaman coffee shop interaktif dalam satu akun.') }}
                    </p>
                    <x-language-switcher />
                </div>
                ...
                <div class="grid grid-cols-3 gap-4 max-w-xl">
                    <div class="rounded-2xl bg-white/10 border border-white/15 backdrop-blur p-4">
                        ...
                        <p class="text-[11px] text-white/70 uppercase">{{ __('Menu curated') }}</p>
                    </div>
                    <div class="rounded-2xl bg-white/10 border border-white/15 backdrop-blur p-4">
                        <p class="text-2xl font-extrabold">5</p>
                        <p class="text-[11px] text-white/70 uppercase">{{ __('Community events') }}</p>
                    </div>
                    <div class="rounded-2xl bg-white/10 border border-white/15 backdrop-blur p-4">
                        <p class="text-2xl font-extrabold">3</p>
                        <p class="text-[11px] text-white/70 uppercase">{{ __('Role portals') }}</p>
                    </div>
                </div>
            </div>
        </section>

<section class="lg:col-span-6 xl:col-span-5 flex items-center justify-center px-5 py-10 sm:px-8 bg-[radial-gradient(circle_at_top_left,#fff_0,#fff8f5_45%,#fdf3e7_100%)]">
...
<a href="{{ route('home') }}" class="text-xs font-bold text-kanca-amber">{{ __('Home') }}</a>
                </div>
...
<p class="mt-6 text-center text-xs text-gray-500">
                    {{ __('Kanca Coffee account access is protected with Laravel authentication and CSRF validation.') }}
                </p>
            </div>
        </section>
    </main>
